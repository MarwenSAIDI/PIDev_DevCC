...
public class serviceconsultation {
    ...
    PreparedStatement pre;
        try {
            pre = con.prepareStatement("INSERT INTO  consultation(id,titre, description,emplacement,image) VALUES ( ?,?, ?, ?,?);");
    ...
    public void update(consultation p)
    {
        PreparedStatement pre;
        try {
            pre = con.prepareStatement( "UPDATE consultation SET titre=?,description=?,emplacement=?,image=? WHERE id=? ;");
            pre.setString(1, p.getTitre());
    pre.setString(2, p.getDescription());
        pre.setString(3, p.getEmplacement());
