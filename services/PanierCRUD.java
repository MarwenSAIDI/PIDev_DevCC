...
public class PanierCRUD {
    ...
    public void Ajouter(Panier pan){
        String sql = "INSERT INTO paniers (cin, Date_C, Status_Panier) VALUES( ?, ?, ?)";
        try {
    ...
    public List<Panier> AfficherPanier() throws SQLException {
        ...
        while(result.next()){
            ...
            p.setID_User(result.getInt("cin"));
    ...
    public void Modifier(Panier pan){
        ...
        String sql = "UPDATE paniers SET cin = ?, Status_Panier = ? WHERE ID_Panier = ?";
        try {
            ste = cnx.prepareStatement(sql);
            ste.setInt(1, pan.getID_User());
    ...
    public List<Panier> verifPanier(int ID_user) throws SQLException{
        String query = "SELECT * FROM paniers WHERE cin = ? AND Status_panier LIKE 'En cours'";
        List<Panier> paniers = new ArrayList<>();
        try {
            ste = cnx.prepareStatement(query);
    ...
    public List<User> getUser(String email){
        String query = "SELECT cin FROM user WHERE email = ?";
